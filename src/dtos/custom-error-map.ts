...
export class ZodErrorMap {
  ...
  public execute(error: Error, ctx: Ctx) {
    ...
    const filteredPath = error.path.filter(
  ...
  private handleInvalidTypeError(error: z.ZodInvalidTypeIssue) {
    ...
    switch (expected) {
      case "string":
        return {
          message: `O campo [${this.currentField}] deve ser uma string`,
        };
  ...
  private handleTooSmallError(error: z.ZodTooSmallIssue) {
  ...
  private handleInvalidStringError(error: z.ZodInvalidStringIssue) {
